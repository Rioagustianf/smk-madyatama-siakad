      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 px-4 bg-white">
          ...
          <div className="flex items-center justify-between w-full">
            <div className="font-semibold">Dashboard Staff Keuangan</div>
            <div className="">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <AvatarWithInitials
                    src={state.user?.avatar}
                    alt={state.user?.name || "Staff"}
                    name={state.user?.name || "Staff"}
                    size="md"
                  />
                </DropdownMenuTrigger>
                <DropdownMenuContent
                ...
                >
                  <DropdownMenuLabel className="text-white">
                    {state.user?.name || "Staff Keuangan"}
                  </DropdownMenuLabel>
                  <DropdownMenuItem
                    className="text-white cursor-pointer"
                    onSelect={() => router.push("/dashboard/staff/profile")}
                  >
